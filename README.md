...
このプロジェクトには自動環境変数チェック機能が組み込まれています。

#### ローカル開発時
- `npm run build` 実行前に自動的に環境変数をチェック
- 必須の環境変数が設定されていない場合、ビルドが中断されます
- `.env` ファイルを確認して必要な環境変数を設定してください

#### Vercel/CI環境
- 環境変数が未設定でも**警告のみ**でビルドが続行されます
- ランタイムで必要な環境変数は、Vercelダッシュボードで設定してください
- 詳細: [Vercel環境変数ドキュメント](https://vercel.com/docs/projects/environment-variables)

#### 環境変数チェックをスキップする（緊急時のみ）
```bash
# 環境変数を設定
SKIP_ENV_CHECK=true npm run build
```

または、Vercelダッシュボードで `SKIP_ENV_CHECK=true` を設定してください。

⚠️ **注意**: この設定は緊急時のみ使用してください。本番環境では適切な環境変数を設定することを強く推奨します。
