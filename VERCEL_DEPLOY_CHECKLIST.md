...
**🆕 2025年改善**: 環境変数チェックスクリプトがVercel環境に対応しました
- Vercel/CI環境では、環境変数が未設定でも**警告のみ**でビルドが続行されます
- ローカル開発では従来通り厳格なチェックが行われます
- `SKIP_ENV_CHECK=true` を設定すれば、環境変数チェックを完全にスキップできます（緊急時のみ推奨）
...
**エラー例**:
```
❌ GEMINI_API_KEY が設定されていません (必須)
❌ DIFY_API_KEY が設定されていません (必須)
```

**解決策**:
```bash
# 方法1: Vercelダッシュボードで環境変数を設定
# Settings → Environment Variables → 必要な変数を追加

# 方法2: 緊急時のみ - 環境変数チェックをスキップ
# Vercel Settings → Environment Variables
# 新しい環境変数を追加:
# Key: SKIP_ENV_CHECK
# Value: true
# ⚠️ この方法は緊急時のみ使用してください
```

**🆕 重要な変更**:
- 2025年版では、Vercel環境では環境変数が未設定でも**警告のみ**でビルドが進みます
- 実行時に必要な環境変数は、アプリ起動後に適切にエラーが表示されます
- ローカル開発では従来通り厳格なチェックが行われます
...
### 4. **"API key is not configured" エラーが出た場合**
